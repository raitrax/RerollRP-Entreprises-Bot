...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption(option => option.setName('target').setDescription(`L'agent à FDA`).setRequired(true)),
  async execute(interaction) {
    ...
    if (membre.roles.cache.has(process.env.SERVICE_ROLE_ID)) {
      ...
      MpEmbed = new EmbedBuilder()
        ...
        .setDescription(`Vous n'avez pas pris votre fin d'astreinte.\nMerci de penser à la prendre à l'avenir !`)
      ...
      FdsEmbed = new EmbedBuilder()
        .setColor(Colors.Red)
        .setAuthor({ name: `Système Iris - ${datetime}` })
        .setDescription(`Fin de l'astreinte par ${nickName}:  **${member.nickname}**`)
